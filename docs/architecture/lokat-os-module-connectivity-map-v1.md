...
- **`src/app/admin/meu-negocio/_client-content.tsx` órfão (severidade P3, confirmado por auditoria independente):** código antigo (BusinessTab/DNA/4Ps/SWOT/Manual) continua compilando mas não é importado por nenhuma rota real desde a sprint do vertical slice Restaurante — a única referência restante no repositório é um teste que lê o arquivo como string para confirmar que continua intocado (`strategy-workspace.structural.test.ts`), nunca um import de produto. Dead end confirmado, não apagado (decisão correta documentada, evita perda de trabalho até uma decisão explícita). **`_client-content.tsx` é um padrão de nome de arquivo comum e majoritariamente VIVO em todo o repositório** (ex.: `src/app/client/home/_client-content.tsx` é importado ativamente por `client/home/page.tsx` como `ClientHomeContent`) — este achado se refere exclusivamente ao arquivo específico de Meu Negócio, nunca ao padrão de nome em si.
