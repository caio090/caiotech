...
| `api/studio/images/generate/route.ts` | POST | REC OS > Studio (Sprint REC OS Studio Image Generation MVP V0.3) | Rota única para o pipeline completo (texto Vidigal + imagem); chamada externa ao provider de imagem já existente e nunca conectado antes desta sprint (`src/lib/ai/image-providers/`, Google Gemini/OpenAI Images); sem persistência (nenhuma tabela nova, assets efêmeros em data: URL, nunca em Storage) |
| `api/rec-os/feed-dna/route.ts` | PUT | REC OS > Studio > Feed DNA (Prompt 16 REC OS Persistence Completion) | Grava override manual em `feed_dna_profiles` (SQL 92) — autoriza a Company via `resolveCompanyContext()` ANTES de gravar, usa sempre o client Supabase da sessão (nunca admin) pra RLS real ser a segunda camada |
| `api/rec-os/series/route.ts` | POST | REC OS > Studio > Série Visual (Prompt 16 REC OS Persistence Completion) | Cria uma Série Visual (`creative_series`) + N `creative_series_items` reais (nunca 1 imagem com N layouts); Company Mode via `resolveCompanyContext()`, Free Mode via `getCurrentUser()` (owner = `created_by`) |
| `api/rec-os/series/[seriesId]/items/[itemId]/route.ts` | PATCH | REC OS > Studio > Série Visual (Prompt 16 REC OS Persistence Completion) | Transição de status de um item da série (planned/generating/ready/error/canceled); autorização real vem de RLS (client da sessão) sobre a série pai; quando `status=ready` com imagem e a série é Company-scoped, tenta persistir o asset em Storage (`client-visual-assets`, SQL 93 — degrada honesto se o bucket ainda não existir) |
