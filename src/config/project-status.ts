...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  },
  {
    ...
    category: "admin",
    description: "Dois controles no header (Painel ADM / Visualizar como) visíveis somente para super_admin, ao lado do sino — src/components/workspaces/workspace-view-switcher.tsx. Nunca troca sessão, cookie ou token; navega para /admin/visualizar com parâmetros que são revalidados no servidor.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Usuário comum, agência e cliente da agência não veem o switcher (gate único: userRole === 'super_admin', mesmo padrão já usado pelo widget de Status V1).",
    next_actions: ["QA local: confirmar ausência do switcher para papéis não super_admin"],
  },
  {
    id: "workspace_preview_security",
    name: "Workspaces — Segurança do modo de visualização",
    category: "admin",
    description: "src/lib/workspaces/preview.ts resolve o preview inteiramente no servidor (nunca confia na query string): confirma super_admin, confirma existência do workspace, confirma relacionamento ativo quando aplicável. Fail-closed — qualquer combinação inválida retorna erro explícito, nunca um preview padrão.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Enforcement de somente-leitura no backend (assertNotPreview, src/lib/workspaces/assert-not-preview.ts) demonstrado em uma rota própria de teste (/api/admin/workspaces/preview-mutation-check) — NÃO retrofitado nas rotas mutáveis reais de Meu Negócio/REC OS/Financeiro nesta sprint. Enquanto isso, um Super Admin em preview já tinha acesso de escrita irrestrito de qualquer forma (papel atual), então isso não é uma escalação de privilégio nova — mas será obrigatório antes de qualquer SupportAccessGrant real (acesso de suporte não-super-admin).",
    next_actions: ["Antes de um primeiro SupportAccessGrant real: retrofitar as rotas mutáveis reais com assertNotPreview", "QA local: confirmar que IDs de workspace adulterados/inexistentes retornam erro, nunca um preview"],
  },
  {
  ...
  },
  {
    ...
    qa: { status: "not_started" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Migration não aplicada, nenhum grant criado.",
  },
  {
