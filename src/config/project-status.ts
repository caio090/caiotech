...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  },
  {
    ...
    category: "admin",
    description: "src/lib/project-status.ts (simples, PROJECT_DEADLINE_V1/MILESTONES_V1-V2) alimentava os badges 'V1 XX%' na interface (_status-client.tsx, _layout-client.tsx) com um V1_PROGRESS hardcoded próprio, divergente do canônico (src/config/project-status.ts). Descoberto durante a recalibração; confirmado por auditoria independente como divergência 'alta e já materializada' (81% na tela vs 65% na recalibração) e severidade P1.",
    phase: "future", readiness: "implemented", qa: { status: "not_started" },
    last_updated: "2026-08-09",
    notes: "Correção pós-auditoria (mesma sprint): src/lib/project-status.ts agora reexporta V1_PROGRESS/V2_PROGRESS de src/config/project-status.ts como fachada temporária (nenhum valor hardcoded independente) -- coberto por src/lib/__tests__/project-status.test.ts. PROJECT_DEADLINE_V1/MILESTONES_V1-V2 permanecem no arquivo simples, fora desta correção. Consolidação completa dos dois arquivos num único sistema ainda recomendada para a Faixa A do roadmap recalibrado. Ver docs/recalibration/lokat-os-recalibration-2026-08.md.",
    priority: "P1",
  },
  {
  ...
  // ── Sprint Gota Neural Foundation V1 — camada de orquestração ──────────
  {
    ...
    category: "admin",
    description: "Contratos TypeScript em src/lib/neural-core/ (Context, Activation, Capabilities, Integrations, Agents, Response Blocks, Actions, Confirmation, Memory, Briefing, Visibility, Planning, LKT/NIS awareness) e Neural Orchestrator determinístico (sem LLM). Cobertura de 31 casos de teste / 72 asserções (capability connection condicional, contract vs runtime, visibility default restritivo, connector events/metrics, planning level/horizon independentes, response block discriminated union real, draft-only actions, no-silent-mutation, memory != conversation).",
    phase: "future",
    readiness: "implemented",
    qa: { status: "not_started" },
    last_updated: "2026-08-09",
    notes: "Ver docs/architecture/gota-neural-foundation-v1.md e ADR-GOTA-NEURAL-001.md. Somente contratos e orchestrator determinístico — nenhuma persistência, LLM, agente executando, integração externa ou UI. V1.1: correções de auditoria independente (CODEX WEB, APPROVED_WITH_CORRECTIONS, 4 P1 + 3 P2) aplicadas -- external audit corrections applied / pending re-audit. Não validated: aguarda re-auditoria externa (CODEX WEB) do delta V1->V1.1 antes de qualquer integração/UI.",
    priority: "P2",
  },
];
