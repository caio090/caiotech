...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  {
    id: "editor_os_layer_scanner",
    name: "EditorOS — Escanear camadas (vertical slice)",
    category: "conteudo",
    description: "Botão 'Escanear camadas' na toolbar do CanvasEditor (visível só com exatamente uma imagem raster selecionada): detecta texto via OCR local, mostra progresso e resultados para revisão, converte regiões aprovadas em EditorElement de texto editável (o modelo real deste editor — CanvasEditor NÃO usa Fabric.js, é um renderer HTML5 <canvas> nativo, sem biblioteca de canvas instalada). Fluxo: detecção → estimativa → revisão humana → conversão. Nunca declara reconstrução perfeita das camadas originais.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1 (hotfixes nas branches fix/editor-os-demo-runtime-v1). Auditoria prévia confirmou (git log --all + busca no working tree) que nenhuma implementação de scanner/OCR/segmentação existia antes nesta sprint — construído do zero, arquitetura em src/lib/editor-os/layer-scanner/. Verificado via script ad-hoc (29 checks): agrupamento de palavras em linhas, mapeamento de coordenadas (escala/rotação/offset, sem NaN), pré-seleção por confiança, capacidades honestas, elegibilidade de limpeza de fundo. Sprint 1.0.1 (QA de dpl_EywfG1V1KsLVXZ2BcbhN4R6DDmUz — P1): a rota /admin/contentos/editor-os retornava 500 sem Supabase configurado porque page.tsx chamava createServerSupabaseClient() incondicionalmente antes de qualquer verificação — corrigido com um runtime de demonstração seguro. Sprint 1.0.2: o Preview publicado com esse hotfix usou o commit e deployment corretos, mas a rota ainda falhou — a causa não foi cache nem arquivo antigo, foi classificação de ambiente errada: demo-runtime.ts reutilizava isProductionEnv() de src/lib/app-url.ts, cujo fallback (NODE_ENV==='production' + APP_URL não-localhost) classifica QUALQUER deployment Vercel como Production, já que a Vercel sempre define NODE_ENV=production (Preview incluído). Corrigido: demo-runtime.ts agora resolve o ambiente sozinho, com VERCEL_ENV como única fonte quando presente — NODE_ENV nunca decide dentro da Vercel. Verificado via 19 checks ad-hoc, incluindo o cenário exato que falhou no QA (Preview + NODE_ENV=production), e via smoke test local real (curl) nas duas rotas. QA local em navegador (Codex Web via servidor local) ainda pendente para este item — nenhuma publicação foi feita nesta sprint.",
    next_actions: ["QA local em navegador (Codex Web via http://127.0.0.1:3000): confirmar que /admin/contentos/editor-os?demo=1 abre o workspace e que a rota sem demo mostra o link seguro, antes de qualquer novo Preview/Production"],
  },
  {
    id: "editor_os_text_detection",
    name: "EditorOS — Detecção de texto (OCR)",
    category: "conteudo",
    description: "OCR local via tesseract.js 7.0.0 (Apache-2.0, único pacote instalado nesta sprint), executado sobre os pixels reais da imagem selecionada (nunca uma captura da página). Worker criado uma vez e reaproveitado durante a sessão, carregado via import dinâmico (lazy — só ao clicar em Escanear camadas), finalizado ao desmontar o editor. Idiomas: português e inglês.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1 (hotfixes nas branches fix/editor-os-demo-runtime-v1). Provider real implementado (não é um placeholder) — Cenário C do prompt (nenhum OCR existia, tesseract.js não estava instalado). Os arquivos do motor (core WASM e dados de idioma) são baixados da CDN pública do tesseract.js na primeira utilização — a imagem do usuário em si nunca é enviada a terceiros, todo o reconhecimento roda no navegador. Sprint 1.0.1: OCR agora também testável em modo demonstração (sem Supabase, fora de Production) — nenhuma alteração no scanner em si, só no runtime que decide se a rota abre. Sprint 1.0.2: a falha vista no QA de Preview não era do OCR — era classificação de ambiente errada em demo-runtime.ts (NODE_ENV='production' na Vercel vale para Preview também); corrigido usando VERCEL_ENV como fonte autoritativa. Nenhuma linha deste item foi alterada. QA local ainda pendente.",
    next_actions: ["QA local em navegador (Codex Web via http://127.0.0.1:3000?demo=1): escanear camadas e confirmar que o OCR funciona sem sessão/Supabase"],
  },
  {
    id: "editor_os_text_conversion",
    name: "EditorOS — Conversão de texto detectado em camada editável",
    category: "conteudo",
    description: "Converte uma região de texto aprovada em um EditorElement real (type: 'text'), com posição/tamanho/rotação mapeados da imagem original para o canvas (coordinate-mapper.ts) e estilo estimado (fonte fallback, tamanho pela altura da região, cor amostrada dos pixels, alinhamento por heurística geométrica) — nunca a fonte original. Escanear + converter entra no histórico de undo/redo como uma única operação composta.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1 (hotfixes nas branches fix/editor-os-demo-runtime-v1). Mapeamento de coordenadas verificado para escala, rotação (90°) e imagem não centralizada — sem NaN. Zoom não entra no cálculo por invariante do próprio CanvasEditor (coordenadas dos elementos são sempre no espaço interno do preset, independente do zoom de exibição). Sprint 1.0.1: conversão agora também testável em modo demonstração — nenhuma alteração na lógica de conversão em si. Sprint 1.0.2: a falha vista no QA de Preview foi causada pelo runtime (Preview classificado como Production por NODE_ENV), não pela conversão de camadas — corrigido em demo-runtime.ts, VERCEL_ENV agora é autoritativo. Nenhuma linha deste item foi alterada. QA local ainda pendente.",
    next_actions: ["QA local em navegador (Codex Web via http://127.0.0.1:3000?demo=1): converter texto detectado em camada"],
  },
  {
    id: "editor_os_layer_review",
    name: "EditorOS — Painel de revisão de escaneamento",
    category: "conteudo",
    description: "Painel lateral (desktop) / bottom sheet (mobile) mostrando progresso por etapa, regiões encontradas com confiança, texto corrigível antes da conversão, e escolha entre manter o fundo original ou tentar a limpeza experimental. Regiões de baixa confiança permanecem disponíveis mas desmarcadas por padrão. Nunca mostra 'objetos' como concluído quando nenhuma segmentação real rodou.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1 (hotfixes nas branches fix/editor-os-demo-runtime-v1). data-testid estáveis (baseados em id gerado, nunca índice): editor-layer-scan-button/panel/progress/result-{id}/convert/cancel/confidence-{id}, editor-layer-panel, editor-layer-item-{id}. Sprint 1.0.1: painel de revisão agora acessível também em modo demonstração; novos data-testid de demo (editor-demo-mode-badge, editor-demo-open-link, editor-demo-clear-draft, editor-demo-import-image, editor-demo-canvas) adicionados para QA local. Sprint 1.0.2: a rota falhou no QA de Preview por classificação de ambiente errada (não por este painel) — corrigido em demo-runtime.ts, VERCEL_ENV agora é autoritativo. QA local ainda pendente.",
    next_actions: ["QA local em navegador (Codex Web via http://127.0.0.1:3000?demo=1): revisar/aprovar regiões de texto"],
  },
  {
    id: "editor_os_layer_serialization",
    name: "EditorOS — Persistência de camadas escaneadas (rascunho local)",
    category: "conteudo",
    description: "Os campos adicionados ao EditorElement (editorOsSource, scannerVersion, sourceObjectId, scanResultId, scanConfidence, originalScannedText, backgroundRemovalMode, hidden, locked) são todos primitivos serializáveis — o saveDraft()/loadDraft() já existentes (localStorage, isolados por clientId) salvam e restauram tudo automaticamente, sem nenhuma alteração nessas funções. Nunca persiste worker, blob temporário, caixa de revisão ou progresso.",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1 (hotfixes nas branches fix/editor-os-demo-runtime-v1). Continua sendo persistência local do navegador (mesma limitação já registrada na área editor_os — sem persistência em nuvem, bloqueada até SQL 87). Sprint 1.0.1: rascunho de demonstração isolado sob chave fixa lokat-editor-os-demo-draft-v1 (sem clientId/contentId/e-mail/nome de cliente), nunca migrado para/de um rascunho autenticado real; ação 'Limpar demonstração' remove só essa chave. Sprint 1.0.2: a falha vista no QA de Preview foi de classificação de ambiente (VERCEL_ENV vs NODE_ENV), não de persistência — corrigido em demo-runtime.ts. QA local ainda pendente.",
    next_actions: ["QA local em navegador (Codex Web via http://127.0.0.1:3000?demo=1): salvar, recarregar e limpar o rascunho de demonstração; confirmar que rascunhos autenticados reais não são afetados"],
  },
  {
    id: "editor_os_object_segmentation",
    name: "EditorOS — Segmentação de objetos não textuais",
    category: "conteudo",
    description: "Recorte de objetos/logos/ícones com máscara precisa (não um recorte retangular genérico). Nenhuma biblioteca de segmentação (SAM, SAM 2, ONNX Runtime Web, OpenCV, rembg, PaddleOCR ou equivalente) está instalada ou implementada neste repositório — confirmado por auditoria de git log --all, package.json/package-lock.json e busca no working tree.",
    phase: "v2",
    readiness: "planned",
    qa: { status: "not_started" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1. Interface e tipos já preparados (DetectedImageLayer/DetectedShapeLayer, LayerScanCapability) para receber um provider real futuramente, sem redesenhar o scanner. Próxima fase precisaria de um modelo de segmentação real (ex.: SAM via ONNX Runtime Web ou um serviço backend) — nunca um crop retangular apresentado como máscara.",
  },
  {
    id: "editor_os_background_reconstruction",
    name: "EditorOS — Reconstrução de fundo após remoção de texto",
    category: "conteudo",
    description: "Reconstrução avançada de fundo (fotos, gradientes, texturas, ilustrações) atrás de um texto removido — não implementada nesta sprint (requires_inpainting, sempre deixado intacto, nenhuma limpeza automática tentada). Uma capacidade menor e diferente — limpeza sólida experimental para fundos quase uniformes (variação de cor abaixo de um limite, com feather via gradiente radial) — está implementada e é oferecida como opção no painel de revisão, claramente marcada 'Experimental'.",
    phase: "v2",
    readiness: "planned",
    qa: { status: "not_started" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/editor-os-layer-scanner-v1. A lógica de elegibilidade (variância de cor) é determinística e foi testada via script ad-hoc; a geração do patch (canvas 2D) só roda no navegador e não pôde ser testada fora dele neste ambiente. Reconstrução real de fundo complexo exigiria inpainting (ex.: um modelo dedicado) — próxima fase.",
  },
  {
  ...
  },
  {
    ...
    notes: "Branch fix/rec-os-global-navigation, sobre o commit 075b023 (já em main/produção). tsc, build e ESLint passam limpos. Isolamento entre clientes verificado apenas em nível de código — sem QA em navegador real com os dois clientes reais (Duh Lanches, O Pedreirão) ainda. Próximo passo: QA funcional autenticado (a Vercel Preview não tem as env do Supabase configuradas) antes de merge em main.",
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-22",
    notes: "Sprint 4.0A.1 corrigiu o P1 relatado (card 'Aguardando aprovação' apontava para /admin/calendario em vez de Aprovações). Com cliente selecionado: aguardando_aprovacao → aprovacoes?status=enviado_aprovacao; em_revisao/alteracoes_solicitadas/em_producao → producao?status=...; agendados → calendário; publicados/em_andamento → resultados. Sem cliente ('todos os clientes'): aguardando_aprovacao/em_revisao/alteracoes_solicitadas apontam para a própria lista 'Precisa da sua atenção' do Hub (#precisa-da-sua-atencao), não mais para o Calendário — é o destino mais honesto disponível, já que aprovacoes/producao/resultados exigem um client e redirecionam para o seletor se ausente (confirmado lendo essas páginas). em_producao/agendados/publicados/em_andamento continuam indo para /admin/calendario, única rota hoje genuinamente cross-cliente. Adicionado suporte mínimo de `?status=` em producao/page.tsx (filtra a lista) e aprovacoes (pré-seleciona a aba mais próxima) — não é filtro exato em todos os casos, documentado no código.",
  },
  {
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-22",
    notes: "Contagem por src/lib/rec-os-hub.ts (bucketContentStatus). Não substitui nem altera /admin/contentos/aprovacoes — só passa a preservar client e um status inicial na URL.",
  },
  {
