...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Usuário comum, agência e cliente da agência não veem o switcher (gate único: userRole === 'super_admin', mesmo padrão já usado pelo widget de Status V1). Hotfix 1.0.1: removido o useEffect que buscava opções de entidade (corrigia um erro novo do ESLint react-hooks/set-state-in-effect); a busca agora roda dentro dos manipuladores de clique. Gap conhecido, não corrigido nesta sprint: se a entrada em preview falhar (ex.: workspace removido), o menu apenas fecha sem mensagem de erro.",
    next_actions: ["QA local: confirmar ausência do switcher para papéis não super_admin", "Adicionar feedback visível quando POST /api/admin/workspaces/preview retornar ok:false"],
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Hotfix 1.0.1 fechou a lacuna divulgada na 1.0: assertWorkspaceMutationAllowed() (src/lib/workspaces/assert-not-preview.ts) não aceita mais um booleano readOnly enviado pelo chamador — deriva o bloqueio sozinho, lendo o cookie assinado e revalidando no banco. Ver workspace_preview_mutation_enforcement para o retrofit real (parcial, 12 rotas) e workspace_preview_audit_log para o contrato de auditoria.",
    next_actions: ["QA local: confirmar que IDs de workspace adulterados/inexistentes retornam erro, nunca um preview", "QA local: confirmar expiração (2h) e revogação (perda do papel super_admin) encerram o contexto"],
  },
  {
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Nenhuma ação do plano foi executada nesta sprint — documento, não script.",
  },
  {
  ...
  // ── Workspaces 1.0.1 (mesma branch feat/workspace-panels-v1) ──────────────
  {
    ...
    category: "admin",
    description: "withMutationProtection/assertWorkspaceMutationAllowed (src/lib/workspaces/assert-not-preview.ts) bloqueiam mutações com 403 WORKSPACE_PREVIEW_READ_ONLY sempre que há preview ativo, derivado só do cookie assinado — nunca de um campo enviado pelo cliente. Retrofitado em 12 rotas reais e existentes: clientes (criar, convite), REC OS (rascunhos criar/editar, enviar p/ aprovação, enviar p/ produção), Equipe (enviar convite por e-mail), Integrações OlaClick (conectar, editar, desconectar) e Financeiro (confirmar pagamento manual, criar cobrança local e via gateway Asaas).",
    phase: "v2",
    readiness: "qa_pending",
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Cobertura PARCIAL e deliberada — amostra representativa priorizada pelas três superfícies de preview, não uma varredura completa. Dezenas de outras rotas mutáveis reais (Meu Negócio: Empresa/DNA/SWOT/Metas/Produto/Serviço/Campanha/Precificação/Fluxo de Caixa; CRM: leads; billing/coupons; rec-projects; contentos/drafts DELETE se existir) permanecem sem o wrapper. Teste ad-hoc cobre só a camada de token/sessão (src/lib/workspaces/__tests__/preview-session.test.ts, 28 asserções) — o guard e o contexto completos (que dependem de cookies()/Supabase/sessão real) não têm teste automatizado nesta sprint.",
    next_actions: [
      "Retrofitar as rotas mutáveis restantes de Meu Negócio, CRM e billing",
      "QA local com sessão real de super_admin: confirmar 403 (não 500) em cada uma das 12 rotas durante preview ativo",
      "Considerar um teste de integração real (servidor + sessão autenticada) para context.ts e assert-not-preview.ts",
    ],
  },
  {
    ...
    qa: { status: "not_started" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Deliberadamente NÃO persiste em tabela — hoje só um console.info(JSON) por evento. Sem SQL criado ou executado nesta sprint; persistência real é trabalho futuro, dependente de aprovação de schema.",
  },
];
