...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  // ── Workspaces 1.0.1 (mesma branch feat/workspace-panels-v1) ──────────────
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-25",
    notes: "Somente na branch hotfix/workspaces-isolation-mobile-v1. Inventário completo em docs/workspace-mutation-inventory.md — toda rota mutável do projeto tem uma classificação, nenhuma ficou pending_protection. Nenhuma Server Action existe no projeto (grep '\"use server\"' vazio). Testes reais: 39 asserções de token + 10 E2E de proxy + 32 confirmando as 8 rotas exigidas pelo QA (403+WORKSPACE_PREVIEW_READ_ONLY+mensagem sanitizada, <1s) + 17 do hotfix 1.0.5 (workspace-picker-source.test.ts, prova estrutural de que os 3 branches source==='blueprint' do picker nunca referenciam hasSupabaseServiceRoleKey/adminDb/createRequiredSupabaseAdminClient) + 12 no hotfix 1.0.6 (layout-shell.structural.test.ts, prova de que o shell real importa/renderiza WorkspaceViewSwitcher e WorkspaceExitButton para desktop e mobile) + 29 novas no hotfix 1.0.7 (13 em access-control-role.test.ts cobrindo resolveEffectiveUserRole(), e +16 ampliando layout-shell.structural.test.ts para provar o encadeamento completo layout server-side → prop initialUserRole → estado inicial do shell (agora uma constante derivada, sem useState/setUserRole) → gates de Painel ADM/Visualizar como/Status V1, incluindo que nenhum token é passado como prop e que o catch da busca de nome não é mais silencioso) + 18 no hotfix 1.0.8 (preview-navigation-sync.structural.test.ts, prova de que router.push()+router.refresh() rodavam nessa ordem, sem gate por pathname e sem loop) — hotfix 1.0.9 REESCREVEU esse mesmo arquivo (não é mais válido testar presença de router.push/router.refresh, já que foram removidos) para 25 asserções provando: zero import de useRouter em ambos componentes, exatamente um window.location.assign/replace por fluxo, ambos gated por res.ok+body.ok (e destination exata na ativação), nenhuma navegação nos blocos catch, nenhum finally envolvendo a saída, proteção contra duplo clique, overlays \"Trocando ambiente...\"/\"Saindo da visualização...\", e Cache-Control: no-store + httpOnly preservado nas 3 respostas de sucesso do POST/DELETE. Total: 164 asserções reais passando + script de cobertura em 40 rotas/0 falhas. /api/admin/contentos/drafts (já uma das 8 rotas do e2e de proxy acima) continua a rota segura reutilizada para o QA Production comprovar o HTTP 403 via navegador — nenhuma rota nova foi criada. O guard por rota (assertWorkspaceMutationAllowed) continua sem teste automatizado de ponta a ponta — precisa de sessão real de super_admin.",
    next_actions: [
