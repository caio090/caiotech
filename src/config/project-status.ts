...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Usuário comum, agência e cliente da agência não veem o switcher (gate único: userRole === 'super_admin', mesmo padrão já usado pelo widget de Status V1). Hotfix 1.0.1: removido o useEffect que buscava opções de entidade (corrigia um erro novo do ESLint react-hooks/set-state-in-effect); a busca agora roda dentro dos manipuladores de clique. Hotfix 1.0.2 (Fase 16): o gap de falha silenciosa foi corrigido — quando POST /api/admin/workspaces/preview retorna ok:false, o menu permanece aberto, mostra uma mensagem (específica para razões conhecidas e seguras, genérica caso contrário) e preserva a seleção para nova tentativa.",
    next_actions: ["QA local: confirmar ausência do switcher para papéis não super_admin", "QA local: confirmar as mensagens de erro específicas com um workspace real removido/desvinculado"],
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Hotfix 1.0.1 fechou a lacuna divulgada na 1.0: assertWorkspaceMutationAllowed() (src/lib/workspaces/assert-not-preview.ts) não aceita mais um booleano readOnly enviado pelo chamador — deriva o bloqueio sozinho, lendo o cookie assinado e revalidando no banco. Hotfix 1.0.2: a chave de assinatura foi isolada (getWorkspacePreviewSigningKey — HKDF-SHA256 a partir de META_APP_SECRET, ou WORKSPACE_PREVIEW_SECRET dedicado quando configurado; falha fechado em Production sem segredo dedicado); cookie inválido/expirado/revogado agora é limpo proativamente ao abrir /admin/visualizar (Fase 17). Ver workspace_preview_mutation_enforcement para o retrofit real e workspace_preview_audit_log para o contrato de auditoria.",
    next_actions: ["QA local: confirmar que IDs de workspace adulterados/inexistentes retornam erro, nunca um preview", "QA local: confirmar expiração (2h) e revogação (perda do papel super_admin) encerram o contexto", "Configurar WORKSPACE_PREVIEW_SECRET na Vercel antes de qualquer uso em Production"],
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Não lê agency_clients para popular uma carteira real ainda — próxima ação depende do provisionamento (docs/workspace-provisioning-plan.md).",
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. O /client/* real não foi alterado — só o preview (/admin/visualizar) aponta para lá com o client_id resolvido.",
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Nenhuma empresa direta real existe no banco ainda — testável hoje apenas via preview de um cliente real marcado como não vinculado a nenhuma agência, ou via blueprint.",
  },
  {
  ...
  // ── Workspaces 1.0.1 (mesma branch feat/workspace-panels-v1) ──────────────
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Inventário completo em docs/workspace-mutation-inventory.md — toda rota mutável do projeto tem uma classificação (protected/not_reachable_from_preview/read_only_operation/demo_memory_only/excluded_with_reason), nenhuma ficou pending_protection. Meu Negócio confirmado demo_memory_only por inspeção (nenhuma escrita em Supabase existe hoje). CRM confirmado not_reachable_from_preview (nenhuma superfície expõe um card CRM ainda). Nenhuma Server Action existe no projeto (grep '\"use server\"' vazio). Testes reais: 39 asserções de token (preview-session.test.ts) + 10 asserções E2E contra um servidor local rodando (proxy-guard.e2e.test.ts, confirma o bloqueio 403 de ponta a ponta sem precisar de login real) + script de cobertura automática (npm run check:workspace-mutations, 0 falhas). O guard por rota (assertWorkspaceMutationAllowed) continua sem teste automatizado de ponta a ponta — precisa de sessão real de super_admin.",
    next_actions: [
      "QA local com sessão real de super_admin: confirmar 403 (não 500) em cada uma das 29 rotas durante preview ativo",
      "Revisar periodicamente docs/workspace-mutation-inventory.md quando novas rotas mutáveis forem adicionadas — o script de cobertura falha automaticamente se uma rota nova não estiver classificada",
