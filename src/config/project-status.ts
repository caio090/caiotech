...
export const PROJECT_AREAS: ProjectAreaStatus[] = [
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Usuário comum, agência e cliente da agência não veem o switcher (gate único: userRole === 'super_admin', mesmo padrão já usado pelo widget de Status V1). Hotfix 1.0.1: removido o useEffect que buscava opções de entidade (corrigia um erro novo do ESLint react-hooks/set-state-in-effect); a busca agora roda dentro dos manipuladores de clique. Hotfix 1.0.2 (Fase 16): o gap de falha silenciosa foi corrigido — quando POST /api/admin/workspaces/preview retorna ok:false, o menu permanece aberto, mostra uma mensagem (específica para razões conhecidas e seguras, genérica caso contrário) e preserva a seleção para nova tentativa. Hotfix 1.0.4 (P1 do QA da 1.0.3): 'Painel ADM' e 'Visualizar como' não cabiam em 390×844 — 6 ícones + busca de w-48 estouravam o header sem quebrar linha. Corrigido reduzindo a busca (w-32 no mobile) e movendo CRM/Status V1 (menor prioridade, não exigidos pelo ticket) para um menu 'Mais' compacto só em mobile, reusando os mesmos handlers/hrefs. Painel ADM e Visualizar como nunca saem da linha principal do header, em nenhum breakpoint. Mensagens de erro do seletor também foram alinhadas ao texto exato pedido: 'Esta empresa não pertence à superfície selecionada.' e 'Não foi possível confirmar a relação entre agência e cliente.'",
    next_actions: ["QA local: confirmar ausência do switcher para papéis não super_admin", "QA local em 390×844: confirmar Painel ADM, Visualizar como e o menu Mais acessíveis sem overflow horizontal"],
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. Hotfix 1.0.1 fechou a lacuna divulgada na 1.0: assertWorkspaceMutationAllowed() (src/lib/workspaces/assert-not-preview.ts) não aceita mais um booleano readOnly enviado pelo chamador — deriva o bloqueio sozinho, lendo o cookie assinado e revalidando no banco. Hotfix 1.0.2: a chave de assinatura foi isolada (getWorkspacePreviewSigningKey — HKDF-SHA256 a partir de META_APP_SECRET, ou WORKSPACE_PREVIEW_SECRET dedicado quando configurado; falha fechado em Production sem segredo dedicado); cookie inválido/expirado/revogado agora é limpo proativamente ao abrir /admin/visualizar (Fase 17). Hotfix 1.0.4: P0 do QA da 1.0.3 corrigido — a superfície direct_business inferia 'sem vínculo em agency_clients = empresa direta', o que expôs clientes reais (Duh Lanches, O Pedreirão) no seletor demonstrativo e suprimia o blueprint. Agora direct_business só aceita o blueprint; resolveWorkspacePreview recusa todo workspace real nessa superfície até existir uma classificação confiável (não roda SQL nesta sprint). P1 também corrigido: WorkspaceContext ganhou relationshipType, e context.ts/preview.ts agora sempre resolvem parentWorkspaceName (blueprint e real) ou falham fechado em vez de deixar o banner mostrar '—' silenciosamente. O banner de preview passou a ser renderizado por src/app/admin/layout.tsx para toda página admin (antes só existia em /admin/visualizar — some ao clicar em qualquer card real do shell). Adicionado reforço contra o cache de back/forward do navegador (evento pageshow/persisted + router.refresh()).",
    next_actions: ["QA local: confirmar que IDs de workspace adulterados/inexistentes retornam erro, nunca um preview", "QA local: confirmar expiração (2h) e revogação (perda do papel super_admin) encerram o contexto", "Configurar WORKSPACE_PREVIEW_SECRET na Vercel antes de qualquer uso em Production", "Aplicar uma classificação real (fora de escopo desta sprint) antes de permitir qualquer empresa direta real no seletor"],
  },
  {
  ...
  },
  {
    ...
    qa: { status: "pending" },
    last_updated: "2026-07-23",
    notes: "Somente na branch feat/workspace-panels-v1. O /client/* real não foi alterado — só o preview (/admin/visualizar) aponta para lá com o client_id resolvido. Hotfix 1.0.4: corrigido o banner mostrando 'Atendido por: —' para o Cliente de Teste 02 (blueprint) — context.ts não resolvia parentWorkspaceName a partir do parentWorkspaceId que já carregava; agora resolve, e o contexto ganhou relationshipType ('managed_client') para o banner exibir a relação, não só o nome.",
  },
  {
    ...
    notes: "Somente na branch feat/workspace-panels-v1/hotfix/workspaces-isolation-mobile-v1. Nota corrigida no hotfix 1.0.4: a versão anterior desta nota dizia 'testável via preview de um cliente real marcado como não vinculado a nenhuma agência' — essa era exatamente a inferência que causou o P0 do QA da 1.0.3 (Duh Lanches e O Pedreirão apareceram como 'empresa direta'). Hoje esta superfície só é testável via o blueprint Empresa/Autônomo de Teste; nenhum workspace real é aceito até existir uma classificação confiável.",
