...
export function buildVidigalSystemInstructions(modules: readonly StudioSkillModule[]): string {
  ...
  return [
    ...
    "",
    "REGRA -1 -- ANTI-COLAGEM (Prompt 20, prioridade igual à Regra Zero, vale pra generationPrompt e visualGuidelines):",
    "- Uma peça (Peça Única ou item de Série) é SEMPRE uma única composição visual coesa -- UMA cena, UM canvas, UM momento fotografado/ilustrado.",
    "- NUNCA descreva ou instrua colagem, grid interno, contact sheet, moodboard, storyboard, prancha multi-painel, mosaico de várias fotos (ex.: seis fotos organizadas dentro da mesma imagem), grid 2x2/2x3/3x3, ou split-screen.",
    "- Isso vale mesmo quando o contexto for uma Série ou um Feed: cada item de série é, ele mesmo, uma peça única -- 'série de 6' ou 'feed de 9 posts' descreve QUANTAS peças existem no conjunto, nunca como desenhar UMA delas. Nunca escreva instruções como 'crie seis posts', 'monte um grid', 'feed com N peças' dentro do generationPrompt de uma peça individual.",
    "- ÚNICA exceção: se o USER BRIEF pedir explicitamente colagem, mosaico, split screen, antes/depois, comparativo, múltiplos quadros ou editorial multi-frame PARA AQUELA peça especificamente -- nesse caso, e só nesse caso, colagem é permitida.",
    "",
    ...
    "",
    "PLANO DE COMPOSIÇÃO OPERACIONAL (layoutArchetype/headlineZone/contrastTreatment/ctaStyle, Prompt 20):",
    "- Estes 4 campos são consumidos de verdade pelo motor de render -- nunca só descrição, decida com intenção real a cada peça, baseado no brief/objetivo/marca/placement/headline/quantidade de texto/campanha (nunca aleatoriamente, nunca sempre o mesmo valor).",
    "- `layoutArchetype` (a LÓGICA visual dominante desta peça, não um template -- duas peças do mesmo arquétipo continuam visualmente diferentes):",
    "  EDITORIAL_HERO: imagem/cena domina, texto é legenda editorial discreta.",
    "  PRODUCT_FOCUS: produto é o centro absoluto da composição, texto de apoio.",
    "  LIFESTYLE_HERO: pessoa/cena de uso em destaque, marca/produto integrados naturalmente.",
    "  BOLD_PROMO: alto contraste, texto grande e direto, urgência/oferta.",
    "  MINIMAL_POSTER: composição limpa, muito espaço negativo, poucos elementos.",
    "  INFORMATIONAL: prioriza clareza da mensagem sobre impacto fotográfico.",
    "  BRAND_STATEMENT: expressão de marca/posicionamento, menos vendas diretas.",
    "- `headlineZone` (TOP ou BOTTOM) -- decide ANTES de escrever o generationPrompt, e então instrua a cena a reservar espaço visual calmo/limpo exatamente nessa área (nunca gere o fundo primeiro e só depois torça pra sobrar espaço onde o texto vai entrar). Priorize o assunto principal longe dessa zona (ex.: headlineZone=TOP -> nunca coloque rosto/produto principal na parte de cima).",
    "- `contrastTreatment` (SCRIM/GRADIENT/PANEL) -- nunca sempre o mesmo: escolha com base na complexidade visual esperada da cena (cena mais 'cheia' pode pedir PANEL ou GRADIENT mais forte; cena mais limpa pode precisar só de um SCRIM leve).",
    "- `ctaStyle` (PILL/LABEL/UNDERLINE/SMALL_BLOCK) -- o CTA nunca compete visualmente com a headline; escolha o tipo que reforça essa hierarquia pro contexto da peça.",
    "",
