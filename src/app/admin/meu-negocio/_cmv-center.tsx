...
type CmvSection = "summary" | "theoretical" | "actual" | "gap" | "menu" | "coverage" | "history";
const SECTION_LABEL: Record<CmvSection, string> = { summary: "Resumo", theoretical: "CMV teórico", actual: "CMV real", gap: "Diferença e investigação", menu: "Engenharia de cardápio", coverage: "Cobertura dos dados", history: "Histórico" };
...
export function CmvCenter({ companyName }: { companyName: string }) {
  ...
  return <div className="space-y-4" data-testid="cmv-center">
    ...
    {section === "theoretical" && <><Panel title="Quanto deveria ter sido consumido (CMV teórico)" subtitle="Quantidade vendida × custo da ficha vigente"><p className="text-2xl font-black">{formatCents(CMV_THEORETICAL.theoreticalConsumption)} · {percent(CMV_THEORETICAL.cmvPercentage)}</p><p className="mt-2 text-xs text-gray-600">Produtos sem ficha entram nas vendas e reduzem a cobertura; nunca recebem custo zero silenciosamente.</p></Panel><CmvProductTable products={CMV_THEORETICAL.products} /></>}
    {section === "actual" && <><Panel title="Quanto realmente foi consumido (CMV real)" subtitle="Estoque inicial + compras − devoluções − estoque final"><p className="text-2xl font-black">{formatCents(CMV_ACTUAL.actualConsumption ?? 0)} · {percent(CMV_ACTUAL.cmvPercentage)}</p><p className="mt-2 text-xs text-gray-600">Transferências entre estoque central e cozinha se anulam no consolidado e não viram consumo duplo.</p></Panel>{manager && <CmvPolicyPanel />}</>}
    {section === "gap" && <><CmvGapPanel /><CmvInvestigationPanel /></>}
    {section === "menu" && <><MenuEngineeringMatrix /><MenuEngineeringTable items={MENU_ENGINEERING} /></>}
