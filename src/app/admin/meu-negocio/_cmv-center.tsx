...
import { CMV_CHANGE_EFFECTS, CMV_EVOLUTION } from "@/lib/business-command-center/fixtures";
...
const QUADRANT_LABEL: Record<MenuEngineeringQuadrant, string> = { star: "Estrela", popular_low_margin: "Muito vendido, mas deixa pouco resultado", profitable_low_popularity: "Rentável, mas pouco conhecido", low_performance: "Baixo desempenho" };
...
function CmvEvolutionReport() {
  return <><Panel title="Evolução do CMV" subtitle="Mensal · fonte demonstrativa · cobertura visível por ponto"><div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={CMV_EVOLUTION}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="period" /><YAxis unit="%" /><Tooltip formatter={(value) => value === null ? "Indisponível" : `${Number(value).toFixed(1).replace(".", ",")}%`} /><Legend /><Line dataKey="actual" name="CMV real" stroke="#dc2626" /><Line dataKey="theoretical" name="CMV teórico" stroke="#7c3aed" /><Line dataKey="target" name="Meta simulada" stroke="#059669" strokeDasharray="4 4" /></LineChart></ResponsiveContainer></div><div className="mt-3 grid gap-2 sm:grid-cols-3">{CMV_EVOLUTION.map((point) => <div key={point.period} className="border p-2 text-[10px]"><strong>{point.period}</strong><p>Cobertura {(point.coverage * 100).toFixed(0)}% · {point.state === "calculated" ? "Calculado" : "Estimado"}</p><p className="text-gray-500">{point.note}</p></div>)}</div></Panel><Panel title="Decomposição da mudança" subtitle="Não afirma causa exata quando faltam dados"><div className="grid gap-2 sm:grid-cols-2">{CMV_CHANGE_EFFECTS.map((effect) => <div key={effect.id} className="border p-3"><div className="flex justify-between gap-2 text-xs"><strong>{effect.label}</strong><span>{effect.percentagePoints === null ? "Incompleto" : `${effect.percentagePoints.toFixed(1).replace(".", ",")} p.p.`}</span></div><p className="mt-1 text-[10px] text-gray-500">{effect.explanation} · {effect.state === "estimated" ? "Estimado" : "Dados insuficientes"}</p></div>)}</div></Panel></>;
}
...
export function MenuEngineeringMatrix() {
  ...
  return <Panel title="Matriz de engenharia de cardápio" subtitle="Popularidade × margem de contribuição · exemplo simulado"><div role="img" aria-label="Matriz de popularidade e margem" className="h-72"><ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{ left: 4, right: 12, top: 8, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" dataKey="x" name="Popularidade" tickFormatter={(value) => `${value}%`} tick={{ fontSize: 10 }} /><YAxis type="number" dataKey="y" name="Margem" tickFormatter={(value) => `R$ ${value}`} tick={{ fontSize: 10 }} /><ReferenceLine x={MENU_THRESHOLDS.popularityThreshold * 100} stroke="#9ca3af" /><ReferenceLine y={MENU_THRESHOLDS.contributionMarginThreshold / 100} stroke="#9ca3af" /><Tooltip cursor={{ strokeDasharray: "3 3" }} formatter={(value, name) => name === "Margem" ? `R$ ${Number(value).toFixed(2).replace(".", ",")}` : `${Number(value).toFixed(1).replace(".", ",")}%`} /><Scatter data={pointsData} name="Produtos analisados" fill="#7c3aed" isAnimationActive={!reduced} /></ScatterChart></ResponsiveContainer></div><div className="mt-2 flex flex-wrap gap-3 text-[10px] text-gray-600"><span><b className="text-emerald-600">●</b> Estrela</span><span><b className="text-amber-600">●</b> Muito vendido, pouco resultado</span><span><b className="text-blue-600">●</b> Rentável, pouco conhecido</span><span><b className="text-gray-500">●</b> Baixo desempenho</span></div></Panel>;
}
...
export function CmvCenter({ companyName, viewMode, onViewModeChange }: { companyName: string; viewMode: BusinessViewMode; onViewModeChange: (mode: BusinessViewMode) => void }) {
  ...
  return <div className="space-y-4" data-testid="cmv-center">
    ...
    {section === "history" && <><CmvEvolutionReport /><Panel title="Histórico" subtitle="Contrato futuro — sem persistência"><div className="flex items-center gap-2 text-xs text-gray-500"><Database className="h-4 w-4" />Nenhum histórico real foi salvo nesta demonstração.</div></Panel></>}
